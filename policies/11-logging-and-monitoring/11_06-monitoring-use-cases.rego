# METADATA
# title: Guardrail 11, Validation 06 - Cloud Platform Monitoring Use Cases
# description: Check for presence of required file(s) in Cloud Storage
package policies.guardrail_11_06_files
...
description := {"description": "validation 06 - Monitoring Use Cases for Cloud Platform"}
...
# description: takes on the value of env var, GR11_06_APPROVAL_FILENAME
#              filename should begin with "06_APPROVAL" but can have different suffix and file type
#              i.e. export GR11_06_APPROVAL_FILENAME='06_APPROVAL_email.pdf'
env := opa.runtime().env
...
# METADATA
# title: Monitoring Use Case Policy - COMPLIANT
...
reply contains response if {
  ...
  contains_approval
	...
	msg := {"msg": sprintf("Required Monitoring Use Case file(s) AND Approval file for [%v, validation %v] detected.", [required_name, validation_number])}
...
reply contains response if {
  ...
  not contains_approval
	...
	msg := {"msg": sprintf("Required Monitoring Use Case file(s) for [%v, validation %v] detected. Approval file NOT detected.", [required_name, validation_number])}
	response := object.union_n([guardrail, status, msg, description, check])
}
...
reply contains response if {
  count(validation_files_list) < required_file_count
	...
	msg := {"msg": sprintf("Required Monitoring Use Case file(s) for [%v, validation %v] NOT detected. Only the following was found: [%v]", [required_name, validation_number, validation_files_list])}
	response := object.union_n([guardrail, status, msg, description, check])
}
