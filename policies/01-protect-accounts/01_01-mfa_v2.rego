...
mfa_users := {asset.users[_].primaryEmail |
  some asset in input#.data
  is_correct_asset(asset)
}

is_enforced(users) if {
  some user in users
  user.isEnforcedIn2Sv
}
  
# get list of all the 'isEnforcedIn2Sv' values for all users
mfa_not_enforced := {asset |#.users[_].primaryEmail |
  some asset in input#.data
  users := asset.users[_]
  is_enforced(users)
  is_correct_asset(asset)
  #asset.users[_].isEnforcedIn2Sv == false
}
...
reply contains response if {
  ...
  #has_false(mfa_enforcement_status)
	...
	msg := {"msg": sprintf("Required MFA Enforcement policy NOT detected for user(s) in [%v, validation %v]. NOT Enforced: [%v]", [required_name, validation_number, mfa_not_enforced])}
	response := object.union_n([guardrail, status, msg, description, check])
}
