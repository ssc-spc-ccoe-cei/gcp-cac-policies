# METADATA
# title: Guardrail 10 - MOU Document
# description: Check for presence of required file(s) in Cloud Storage
package policies.guardrail_10_files
...
# METADATA
# title: MOU Document - COMPLIANT
# description: If matching_assets is TRUE (all files found) then reply back COMPLIANT
reply contains response if {
	matching_assets
	status := {"status": "COMPLIANT"}
	check := {"check_type": "MANDATORY"}
	msg := {"msg": sprintf("Required [MOU Document] file for [%v] detected.", [required_name])}
...
reply contains response if {
	...
	check := {"check_type": "MANDATORY"}
	msg := {"msg": sprintf("Required [MOU Document] file(s) for [%v] NOT detected.", [required_name])}
	response := object.union_n([guardrail, status, msg, description, check])
}
