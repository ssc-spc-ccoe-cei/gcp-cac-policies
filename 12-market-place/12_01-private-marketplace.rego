# METADATA
# title: Guardrail 12, Validation 01 - Third Party Marketplace Restrictions
# description: Check for presence of required file(s) in Cloud Storage
package policies.guardrail_12_01_files
...
description := {"description": "Third Party Marketplace Restrictions"}
...
# METADATA
# title: Third Party Marketplace Restriction Policy - COMPLIANT
...
reply contains response if {
  ...
  contains_approval
	...
	msg := {"msg": sprintf("Required Third Party Marketplace Restriction file(s) AND Approval file for [%v, validation %v] detected.", [required_name, validation_number])}
...
reply contains response if {
  ...
  not contains_approval
	...
	msg := {"msg": sprintf("Required Third Party Marketplace Restriction file(s) for [%v, validation %v] detected. Approval file NOT detected.", [required_name, validation_number])}
	response := object.union_n([guardrail, validation, status, msg, description, check])
}
...
reply contains response if {
  ...
  check := {"check_type": "MANDATORY"}
	...
	msg := {"msg": sprintf("Required Third Party Marketplace Restriction file(s) for [%v, validation %v] NOT detected. Only the following was found: [%v]", [required_name, validation_number, validation_files_list])}
	response := object.union_n([guardrail, validation, status, msg, description, check])
}
